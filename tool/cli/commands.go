...
//
// API "ISprime Origins API": CLI Commands
//
// Command:
// $ goagen
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "show",
		Short: `Show an image metadata`,
	}
	tmp1 := new(ShowImageCommand)
	sub = &cobra.Command{
		Use:   `image ["/api/images/ID"]`,
		Short: ``,
	...
	tmp2 := new(UploadImageCommand)
	sub = &cobra.Command{
		Use:   `image ["/api/images/"]`,
		Short: ``,
...
func (cmd *ShowImageCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/api/images/%v", cmd.ID)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
